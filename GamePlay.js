...
class gameMap{
  ...
  coin[] coins = {new coin(100,100),new coin(100,400), new coin(200,200)};
  ...
  gameMap(pIn, cIn, gIn){
	  ...
	  for (int i=0; i< pIn.length; i++){
		...
		wB = pIn[i];
		console.log(wB);
		wallBlocks = append(wallBlocks, new wallBlock(int(wB[0]), int(wB[1]), 100, 50));
	  }
	  console.log("coins:");
	  coins = {};
	  for (int i=0; i< cIn.length; i++){
		...
		wc = cIn[i];
		console.log(wc);
		coins = append(coins, new coin(int(wc[0]), int(wc[1])));
	  }
	  console.log("goal:");
	  g = gIn[0]
	  console.log(g);
	  goal = new wallBlock(g[0], g[1], 100,50);
